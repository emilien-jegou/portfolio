...
data:
    published: false
    title: "Pathing down from function classes"
...
One of the challenge with dealing with inheritance is detecting whether a class extends from another or in other words the `instanceOf` function, it's a bit of an unelegant way to handle class polymorphism but there may be time where it's your only option.

There's actully nothing magic about instanceOf, for every class defined we append some information payload through the constructor. We can copy the same behavior with a helper function.

As such I also coded a version of this code that allow for this kind of dynamic cast through a cls utility, you can find [it in this gist](https://gist.github.com/emilien-jegou/4ce3e69fb0bcfc047274bda6535df408).
...
Functional programming principles have gained significant footing in the programming community in that aspect, Elixir and Rust both have a dedicated loving fanbase as "in between paradigms" languages, while concepts from FP languages like haskell sips into other languages, sometimes with different names, e.g. haskell `sum types` are available through `tagged unions` in typescript, while concepts like the reader monad reappears through `pipe` utilities.
...
const createNinja = (ctx: Context<{ ninjaWeapon: Weapon }>) => ({
  slash(): number {
