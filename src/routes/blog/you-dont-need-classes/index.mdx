...
Since their "shinier" counterpart arrived, function classes have slowly faded away, and OOP practices have thrived to this day through libraries like Angular and Nest.js, but what does this new addition really bringed to the language? This article is an opinionated one that wish to give you a different perspective on OOP and programming in typescript in general, my wish is to give a more nuance take about what OOP is really giving you with the hope of broadening your horizon in a sort of article/tutorials kinda way. — enjoy!

## "Your code doesn't allow X"

A common criticism of class-based programming is its tendency toward
over-engineering, in that regards, converting class-heavy code to more
functional approaches frequently results in significant code reduction, I will credit two reasons for this: programming practices and an "architectural"-first mindset.

Over-engineering solutions often come from a place of anticipating future
extensions or requirements, in OOP this tendancy could partly be explained by
the highly codified practices developers have to follow (`SOLID`, `DRY`,
design patterns and so on). While in isolation thoses rules may appears appealing, in combination they often lead to over-abstraction, on any single piece of code the question "*which of SOLID or DRY principles could this code benefit from*" would lead up to an answer.

The `Single Responsibility principles` is notable for leading developers to premature optimization, as abstractions can always be justified by the rule: my class does X and Y therefore it has two responsibility or two reason to exist. When it was created the rule didn't provide a "bad reason" for abstraction, this is obviously an issue as the benefit of abstraction are much more nuanced that the rule would let it believe, to give a parallel it would be like asking developers to drink more water as it's good for them, just for them to suffer from water intoxication, not so strangely developers are very good at seeing things in a binary approach.

<GetterAndSetterImage />
[Getter and setter, or an example of how programming concepts could lead to premature optimizations](https://stackoverflow.com/questions/1568091/why-use-getters-and-setters-accessors)

What you get from abstraction is isolation of changes, yet a more efficient approach would be implementing these hypothetical extensions only when genuinely required, *through focused refactoring*, or not be afraid of changes.

SOLID gained popularity in the early 2000s but with the advent
of LSP and types systems refactoring has never been made easier, in 2004
languages made extra effort to not have a type-system (think python, ruby,
PHP), in that context any security guarantees in your code were extrapolated
to extreme importance. Without strong typing it becomes trivial for developers to breaks things, and things were broken all the time, and such we created a platform for code to be extended rather than modified, as such, things that already worked wouldn't have to be revisited, we would just build on top of them or forced extensibility accross most objects through diverse patterns.

A more pragmatic "only what I need" development strategy keeps codebases cleaner, simpler, and more maintainable rather than providing functionality based on speculative gains, for you to know what is speculative and what is not you will need to question the fundamentals, but do not fear refactoring.

*why are you telling me this?* -- Well I wan't to be clear about my intent, in this article I will not provide a 1 to 1 representation of javascript classes to their "function" counterpart, instead I will simply reimagine the code from the OOP implementation through the lens of functional programming: or in other word, *if the code doesn't allow X it's because it does not need X right now*.

For the same reason, when doing side-by-side in this article, I will not adhere to any strict coding rules on the object-oriented side, e.g. I will not mark all variable as privates to respect the Open-closed principle.
...
// the line // (e.g. instanceof would return `Object` on
// mockDatabase instead of `SaveManager`)
...
class Profile {
  ...
  // but it is not nullable in the type. Decorators fundamentally operates
