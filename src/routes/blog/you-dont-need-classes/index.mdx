...
dynamic programming languages thrived (think python, ruby, PHP), in that
context any security guarantees in your code were extrapolated to extreme
importance. Without strong typing it becomes trivial for developers to breaks
things, as such Robert C. Martin created SOLID which is essentially a platform
for code to be extended rather than modified, why care if developers became too
strict with the rule implementation when the opposite approach could lead to
disastrous bugs?

Assuming you possess and respect a strongely typed approach, a more pragmatic
"code what you need" or "keep it simple" development strategy will keep your
codebase cleaner, simpler, and more maintainable; instead of focusing your
effort on speculatives principles, focus on the gain on what you need right
now and learn not to fear code iterations.

*why are you telling me this?* -- Well I wan't to be clear about my intent, in this article I will not provide a 1 to 1 representation of javascript classes to their "function" counterpart, instead I will simply reimagine the code from the OOP implementation through the lens of functional programming: or in other word, if you think the code need X you should be able to showcase it.

One example looking at the functional classes example could be: "your code doesn't allow method overwrite because the function is outside of the class", in this case, we would assume that the method *needs* to be overwritten which could be showcase by:
- a class currently needing to overwrite the method
- the class is exposed to an external interface and we want our users to be able to extend it.
