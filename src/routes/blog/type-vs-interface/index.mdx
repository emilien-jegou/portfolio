...
Typescript allow us to use `interface` and `type` intercheangably, we tend to favorise `type` as they represent a higher subset of features, e.g. you can represent inheritance with both interface and types but only with the second can you perform type mapping e.g. `Partial` or `Omit`.
